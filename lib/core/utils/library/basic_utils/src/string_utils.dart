...
class StringUtils {
  ...
  /// Example : hello => olleh
  ///
  static String reverse(String s) => String.fromCharCodes(
        s.runes.toList().reversed,
      );

  ///
  /// Counts how often the given [char] appears in the given string [s].
  ...
  static int countChars(String s, String char, {bool caseSensitive = true}) {
    int count = 0;
    s.codeUnits.toList().forEach((int i) {
      if (caseSensitive) {
  ...
  static String addCharAtPosition(
    ...
    if (!repeat) {
      ...
      final String before = s.substring(0, position);
      final String after = s.substring(position, s.length);
  ...
  static List<String> chunk(String s, int chunkSize) {
    final List<String> chunked = <String>[];
    for (int i = 0; i < s.length; i += chunkSize) {
      final int end = (i + chunkSize < s.length) ? i + chunkSize : s.length;
  ...
  static String generateRandomString(
    ...
    final List<String> possible = res.split("")..shuffle();
  ...
  static String _randomizer(
    ...
    const String a = "ABCDEFGHIJKLMNOPQRXYZ";
    const String la = "abcdefghijklmnopqrxyz";
    const String b = "0123456789";
    const String c = r"~^!@#$%^&*;`(=?]:[.)_+-|{}";
  ...
  static List<String> generateRandomStrings(
    ...
    for (int i = 0; i < amount; i++) {
      final String s = generateRandomString(
        length,
        alphabet: alphabet,
        numeric: numeric,
        special: special,
